...
function drawScene(
  ...
  for (let gy = 0; gy < world.height; gy++) {
    for (let gx = 0; gx < world.width; gx++) {
      ...
      // Rival clubs, fog-gated to the three-tier model. A rival HQ is a fixed
      // landmark (like a hockey org): shown on any EXPLORED tile, dimmed to
      // "memory" when not currently in sightline. Rival units MOVE every month,
      // so they only render where the player has CURRENT vision — otherwise the
      // player would see live enemy positions on tiles they explored long ago.
      if (explored) {
